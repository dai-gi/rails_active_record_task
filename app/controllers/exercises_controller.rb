class ExercisesController < ApplicationController
  ...
  def exercise3
    ...
    @address = Address.joins(:orders).where(postalcode: Address.joins(:orders).group(:postalcode).order('count_all DESC').count.first.first).pluck(:postalcode, :prefecture, :city, :line_1, :line_2)
  end
